...
THRESHOLDS = {
    # Large Cap (High Volume, Lower Volatility)
    "BTC": 0.015,  # 1.5% (Any move > $6 triggers a trade)
    "ETH": 0.015,  # 1.5% (Any move > $4.50 triggers a trade)
    # Stablecoin / Gold
    "USDT": 0.015,  # 1.5% (Keep cash tight)
    "PAXG": 0.02,  # 2.0% (Gold is slower, give it breathing room)
    "XAUT": 0.02,  # 2.0% (Same as PAXG)
    # Altcoins (High Volatility)
    # If you set these to 1.5%, you will trade too often and pay too many fees.
    # 3% is the "Aggressive" sweet spot for Alts.
    "SOL": 0.03,  # 3.0% (Any move > $3 triggers a trade)
    "BNB": 0.03,  # 3.0%
    "XRP": 0.03,  # 3.0%
}
