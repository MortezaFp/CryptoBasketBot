...
def run_swing_cycle(api=None):
    ...
    notifier = TelegramNotifier(telegram_token, chat_id) if telegram_token and chat_id else None
    ...
    balances = {k: Decimal(str(v.get("value", 0))) for k, v in balances_resp.get("result", {}).get("balances", {}).items()}
    ...
    for coin in TARGET_COINS:
        ...
        if state == "OUT":
            math_check = (last_row['ema_9'] > last_row['ema_21']) and (last_row['rsi'] > 50) and (last_row['close'] > last_row['sma_200'])
            if math_check:
                ...
                if ai_resp and ai_resp.get('signal') == "BUY":
                    ...
                    if intended_size > 0:
                        actual_trade_size = min(intended_size, current_bank * MAX_BANK_ALLOCATION_PCT, current_bank)
                        if actual_trade_size >= Decimal("10.0"):
                            logger.info(f"Executing BUY for {coin}. Size: {actual_trade_size}")
                            ...
                            try:
                                ...
                                qty = amount_to_buy.quantize(Decimal(precision_str), rounding=ROUND_DOWN)
                                
                                api.create_order(f"{coin}USDT", "BUY", qty, type="MARKET")
                                current_bank -= actual_trade_size
                                summary_messages.append(f"✅ BUY {coin}: ${actual_trade_size:.2f} @ {current_price:.2f} (AI Conf: {conf}) - {ai_resp.get('reason')}")
                            except Exception as e:
            ...
            if not last_order:
                logger.warning(f"Could not find entry price for {coin}. Skipping IN evaluation.")
                continue
            ...
            sell_condition = (
                (current_price <= dynamic_stop_loss) or
                (current_price >= take_profit) or
                (last_row['ema_9'] < last_row['ema_21'])
            )
            
            if sell_condition:
                logger.info(f"Executing SELL for {coin}. Condition met. Price: {current_price}, Stop: {dynamic_stop_loss}, TP: {take_profit}")
                try:
                    precision = api.get_market_precision(f"{coin}USDT")
                    precision_str = ("0." + "0" * (precision - 1) + "1" if precision > 0 else "1")
                    qty = coin_balance.quantize(Decimal(precision_str), rounding=ROUND_DOWN)
                    
                    api.create_order(f"{coin}USDT", "SELL", qty, type="MARKET")
                    summary_messages.append(f"❌ SELL {coin}: {qty} @ {current_price:.2f} (Entry: {entry_price:.2f})")
                except Exception as e:
